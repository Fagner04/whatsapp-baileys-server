...
async function waitForQR(session, maxWaitMs = 60000) {
  ...
  while (Date.now() - startTime < maxWaitMs) {
    ...
    if (elapsed > 0 && elapsed % 5000 < 300) {
      console.log(`⏱️ ${Math.floor(elapsed/1000)}s - Status: ${status}, QR: ${!!qr}`);
    }
  ...
  console.error(`❌ Timeout após ${maxWaitMs}ms - Status: ${session.getStatus()}`);
  throw new Error('Timeout ao gerar QR Code - WhatsApp demorou mais de 60 segundos. Tente novamente em alguns segundos.');
}
...
async function initWhatsApp(sessionId) {
  try {
    ...
    if (!fs.existsSync(sessionDir)) {
      ...
      console.log(`[${sessionId}] 📁 Usando credenciais salvas em: ${sessionDir}`);
    }

    // Carregar versão mais recente
    console.log(`[${sessionId}] 📦 Carregando versão do Baileys...`);
    const { version, isLatest } = await fetchLatestBaileysVersion();
    console.log(`[${sessionId}] 📦 Versão: ${version.join('.')}, Latest: ${isLatest}`);
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (qr) {
        try {
          console.log(`[${sessionId}] 📱 QR Code recebido! Convertendo para DataURL...`);
          qrCode = await QRCode.toDataURL(qr);
          connectionStatus = 'qr_ready';
          console.log(`[${sessionId}] ✅ QR Code gerado com sucesso! Length: ${qrCode.length}`);
        } catch (err) {
      ...
      if (connection === 'close') {
        ...
        console.log(`[${sessionId}] DisconnectReason.loggedOut:`, DisconnectReason.loggedOut);
        ...
        if (shouldReconnect) {
          ...
          setTimeout(() => {
            ...
            initWhatsApp(sessionId).catch(err => {
              console.error(`[${sessionId}] ❌ Erro ao reconectar:`, err.message);
              connectionStatus = 'disconnected';
            });
          }, 3000);
        } else {
          // Só deletar credenciais se o usuário fez logout explícito
          console.log(`[${sessionId}] 🗑️ Logout detectado - removendo credenciais`);
          connectionStatus = 'disconnected';
...
app.post('/whatsapp/init', async (req, res) => {
  ...
  try {
    ...
    if (!session) {
      ...
      const status = session.getStatus();
      if (status === 'connected') {
        console.log(`[${sessionId}] ✅ Reconectado automaticamente com credenciais salvas!`);
        return res.json({
          success: true,
    ...
    console.log(`[${sessionId}] Sessão existente - Status: ${status}, QR: ${!!qr}`);

    res.json({
      success: true,
      message: qr ? 'QR Code disponível' : status === 'connected' ? 'Já conectado' : 'Aguardando conexão',
      qr,
      status,
      phoneNumber: session.getPhoneNumber()
    });
...
app.post('/whatsapp/send', async (req, res) => {
  try {
    const { phone, message, sessionId = 'default', mediaUrl, caption } = req.body;
    console.log(`📤 [SEND] Tentando enviar para ${phone}`);
    console.log(`📤 [SEND] Mídia: ${mediaUrl ? 'SIM' : 'NÃO'}, Legenda: ${caption ? 'SIM' : 'NÃO'}`);
    ...
    if (currentStatus !== 'connected') {
      console.error(`❌ [SEND] WhatsApp não conectado. Status: ${currentStatus}`);
      return res.status(400).json({
        success: false,
    ...
    try {
      ...
      if (result && result.exists) {
        ...
        console.warn(`⚠️ [SEND] Número não encontrado no WhatsApp, tentando enviar mesmo assim`);
      }
    } catch (verifyError) {
      console.warn(`⚠️ [SEND] Erro ao verificar número, continuando com JID padrão:`, verifyError.message);
    }
    ...
    // Se tiver mídia (imagem), enviar como imagem com legenda
    if (mediaUrl) {
      console.log(`📸 [SEND] Baixando imagem: ${mediaUrl}`);
      ...
      try {
        ...
        console.log(`✅ [SEND] Imagem baixada: ${imageBuffer.length} bytes`);
        
        // Preservar quebras de linha na legenda
        const finalCaption = caption ? caption.replace(/\\n/g, '\n') : message || '';
        console.log(`📝 [SEND] Legenda (${finalCaption.length} caracteres)`);
        
        // Enviar imagem com legenda
        sendResult = await session.sock.sendMessage(jid, {
          image: imageBuffer,
          caption: finalCaption,
        });
        
        console.log(`✅ [SEND] Imagem com legenda enviada!`);
      } catch (imageError) {
        console.error(`❌ [SEND] Erro ao enviar imagem:`, imageError);
        throw new Error(`Falha ao enviar imagem: ${imageError.message}`);
      }
    } else {
      // Enviar apenas texto (preservar quebras de linha)
      const finalMessage = message ? message.replace(/\\n/g, '\n') : '';
      console.log(`📝 [SEND] Mensagem de texto (${finalMessage.length} caracteres)`);
      
      sendResult = await session.sock.sendMessage(jid, { text: finalMessage });
      console.log(`✅ [SEND] Mensagem de texto enviada!`);
    }
    ...
    res.json({
      success: true,
      message: mediaUrl ? 'Imagem enviada com sucesso' : 'Mensagem enviada',
  ...
  for (const [sessionId, session] of sessions.entries()) {
    ...
    if (session.sock && status === 'connected') {
      try {
        ...
        console.log(`[${sessionId}] ✅ Keep-alive OK - Status: ${status}, Phone: ${session.getPhoneNumber()}`);
      } catch (error) {
        console.error(`[${sessionId}] ❌ Erro no keep-alive:`, error.message);
        console.log(`[${sessionId}] Tentando reconectar após erro de keep-alive...`);
      ...
      console.log(`[${sessionId}] ⚠️ Sessão não conectada - Status: ${status} - Tentando reconectar...`);
